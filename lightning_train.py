...
class UNetSegmentation(pl.LightningModule):
    ...
    def test_step(self, test_batch, batch_idx):
        ...
        acc = pure_mask.eq(pure_predict).sum().item()/len(pure_mask)
    ...
    def validation_epoch_end(self, outputs):
        ...
        """
        avg_loss = torch.cat([x["val_step_loss"] for x in outputs], dim=0).mean()
        avg_acc = torch.cat([x["val_step_acc"] for x in outputs], dim=0).mean()
    ...
    def test_epoch_end(self, outputs):
        ...
        """
        avg_test_acc = torch.cat([x["test_acc"] for x in outputs], 0).mean()
